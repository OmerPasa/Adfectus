...
public class HumanBossAttackInitiater : HumanBossBaseState
{
    ...
    {
        if (boss.isAttackingMedium == false && boss.isAttackingShort == false)
        {

            //boss.timeBtwAttack ı mı silsek ?
            Deb.ug("İnitiating Attack");
            if (Vector3.Distance(boss.transform.position, boss.character.transform.position) <= boss.meleeRange && boss.timeBtw_shortAttack <= 0 && boss.isAttackingMedium == false)
            {
                boss.SwitchState(boss.meleeState);
                Debug.Log("melee attack");
            }
            else if (Vector3.Distance(boss.transform.position, boss.character.transform.position) <= boss.mediumRange && boss.timeBtw_midAttack <= 0 && boss.isAttackingShort == false)
            {
...
public class HumanBossMediumState : HumanBossBaseState
{
    ...
    public override void EnterState(HumanBossController boss)
    {
        Deb.ug("staring to fire fireballs.");
